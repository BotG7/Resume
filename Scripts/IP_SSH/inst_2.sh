...
set_proxy() {
    ...
    if [ "$mode" == "2" ]; then
        ...
        ssh -o StrictHostKeyChecking=no $user@"$ip" "gsettings set org.gnome.system.proxy mode 'manual' && 
        ...
        ssh -o StrictHostKeyChecking=no $user@"$ip" "gsettings set org.gnome.system.proxy mode 'none'"
